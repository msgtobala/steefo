...
/** Top-right diagonal cut — same mask as HomeProjects carousel media. */
const mediaClip =
...
/**
 * Home products carousel — same structure/layout as HomeProjects.
 * Active card grows while the track scrolls with the same tween.
 */
export function HomeProducts({ products, className }: HomeProductsProps) {
  ...
  function goToIndex(nextIndex: number) {
    ...
    if (!scroller) return
    const to = Math.min(products.length - 1, Math.max(0, nextIndex))
    if (to === activeIndexRef.current) return

    const cards = Array.from(
      scroller.querySelectorAll<HTMLElement>('[data-home-product-card]'),
    )
    ...
    const startH = cards.map((card) => {
      const media = card.querySelector<HTMLElement>('[data-home-product-media]')
    ...
    >
      <Container>
        <div className="flex flex-col gap-8 lg:flex-row lg:items-start lg:justify-between lg:gap-12">
          <div className="max-w-[400px] shrink-0" data-home-products-header>

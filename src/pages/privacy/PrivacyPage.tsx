...
import { privacyStrings } from '../../resources/privacy_strings'
...
/**
 * Privacy Policy — same legal layout as Terms.
 * Content from https://www.steefo.com/privacy-policy/
 */
export function PrivacyPage() {
  const rootRef = usePageRevealRef({ withHero: true })
  const { eyebrow, title, titleLines, intro, sections } = privacyStrings
                ...
                <div className="space-y-8 px-5 py-6 md:space-y-10 md:px-8 md:py-8">
                  {section.blocks.map((block, index) => {
                      ...
                      return (
                        ...
                        >
                          {block.items.map((item) => (
                            ...
                            >
                              <BrandArrow />
                              <p className="font-display text-base leading-[1.55] text-body">
                                {'label' in item && item.label ? (
                                  <>
                    ...
                    if (block.type === 'cookies') {
                      return (
                        <div
                          key={`${section.id}-cookies-${index}`}
                          className="space-y-4"
                        >
                          {block.items.map((item) => (
                            <div
                              key={item.title}
                              className="border border-border bg-surface-muted/50 px-4 py-5 md:px-5"
                            >
                              <div className="flex items-start gap-2.5">
                                <BrandArrow className="mt-1" />
                              ...
                              <dl className="mt-4 space-y-2 pl-5.5 font-display text-sm leading-[1.5] text-body md:text-base md:leading-[1.55]">
                                <div>
                                  <dt className="inline font-semibold text-foreground">
                                    Type:{' '}
                                  </dt>
                                  <dd className="inline">{item.cookieType}</dd>
                                </div>
                                <div>
                                  <dt className="inline font-semibold text-foreground">
                                    Administered by:{' '}
